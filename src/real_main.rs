    ...
    thread::spawn(move || {
        ...
        if let Some(window) = weak2.upgrade() {
            loop {
                ...
                while let Some(ref mut rec_from_drones) = *general_receiver2.lock().unwrap() {
                    // let mut select = Select::new();
                    // let oper1 = select.recv(&rec_from_drones); // Blocks until a message is available
                    // let message = select.select();
                    println!("[SIMULATION-CONTROLLER] Waiting for message");
                    match &rec_from_drones.recv(){
                        ...
                        Ok(DroneEvent::ControllerShortcut(packet)) => {
                            ...
                            if let Some(ref mut s) = *channels1.lock().unwrap() {
                                if let Some(channel) = (*s).get(&destination) {
                                    channel.sender.send((*packet).clone());
                                } else {
                        ...
                        _ => {
                            println!("[SIMULATION CONTROLLER] Unknown message");
                        }
                    }
                // } else {
                //     println!("[SIMULATION CONTROLLER] No receiver");
                }
            }
        }
    });
